/* scorpion OS - kernel/descriptor_tables.c
   GDT (3 entries: null, code, data) and IDT (256 entries)  */
...
static void idt_set_gate(uint8_t num, uint32_t base, uint16_t sel, uint8_t flags) {
    ...
    idt_entries[num].flags     = flags; /* 0x8E = present, DPL=0, 32-bit interrupt gate */
}
...
void isr_handler(registers_t *regs) {
    if (interrupt_handlers[regs->int_no]) {
        ...
        terminal_printf("[ISR] Unhandled exception %d (err=%x)\n",
                        regs->int_no, regs->err_code);
    }
}

/* Called from ASM irq_common_stub */
void irq_handler(registers_t *regs) {
    /* Call handler first, then send EOI */
    if (interrupt_handlers[regs->int_no])
        interrupt_handlers[regs->int_no](regs);

    /* Send EOI to PIC(s) */
    if (regs->int_no >= 40) outb(0xA0, 0x20); /* slave PIC EOI */
