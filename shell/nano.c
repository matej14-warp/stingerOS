...
void nano_open(const char *filename, sfs_node_t *cwd) {
    ...
    if (vbe_screen_height > 0) NANO_ROWS = vbe_screen_height / 16 - 2;
    ...
    while (1) {
        ...
        if (c == 0x17) { do_find(); redraw(); continue; }
        if (c == 0x0B) {
        ...
        if (c == 0x1b) {
            ...
            if (c2=='[') {
                ...
                } else if (c3=='H') { cur_col=0; }
                else if (c3=='F') { cur_col=slen(lines[cur_row]); }
                else if (c3=='5') { keyboard_getchar();
                    cur_row-=NANO_ROWS; if(cur_row<0)cur_row=0; cur_col=0;
                } else if (c3=='6') { keyboard_getchar();
...
void nano_open_gui(const char *filename, sfs_node_t *cwd) {
    ...
    while (1) {
        ...
        if (c == 0x13) {
            if (!nano_filename[0]) {
                ...
                extern file_dialog_result_t file_dialog_save(sfs_node_t *start_dir, const char *title, const char *default_name);
                file_dialog_result_t res = file_dialog_save(cwd, "Save File", "untitled.txt");
                if (res.result) {
                    sncpy(nano_filename, res.filename, 128);
                }
            }
            if (nano_filename[0]) {
                save_file(cwd);
