    ...
    if (g_shell_parts[i].drive == drive && g_shell_parts[i].index == idx) {
      ...
      if (drive >= SATA_DRIVE_BASE)
        r = ahci_write_sectors(drive - SATA_DRIVE_BASE,
                               (uint64_t)g_shell_parts[i].lba_start, 1, buf);
      else
...
static void cmd_install(void) {
  ...
  if (pd->size_lba < ps->size_lba) {
    terminal_printf("  ERROR: Destination partition is smaller than source!\n");
    return;
  }
  ...
  for (uint32_t c = 0; c < chunks; c++) {
    int read_ok =
        s_sata
            ? ahci_read_sectors(s_port, ps->lba_start + c * 8, 8, copy_buf) == 0
            : ata_read_sectors(s_port, ps->lba_start + c * 8, 8, copy_buf) == 0;
    if (!read_ok) {
      terminal_printf("  Read error at chunk %u\n", c);
      return;
    }

    int write_ok = d_sata ? ahci_write_sectors(d_port, pd->lba_start + c * 8, 8,
                                               copy_buf) == 0
  ...
  if (rem > 0) {
    ...
    int read_ok =
        s_sata
            ? ahci_read_sectors(s_port, ps->lba_start + off, rem, copy_buf) == 0
            : ata_read_sectors(s_port, ps->lba_start + off, rem, copy_buf) == 0;
    if (read_ok) {
      if (d_sata)
  ...
  terminal_printf("  Do you want to set up a SYNC_TARGET partition? [Y/n] ");
  char yn[4];
  readline(yn, 4);
  if (yn[0] != 'n' && yn[0] != 'N') {
    terminal_printf(
        "  Create a [N]ew partition in empty space, or use [E]xisting? [N/e] ");
    readline(yn, 4);
    int sync_pi = -1;
    ...
    if (yn[0] == 'e' || yn[0] == 'E') {
      ...
      terminal_printf("  Will create on drive %d. Start LBA (e.g. %u): ", dd,
      ...
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < g_shell_part_count; j++) {
          if (g_shell_parts[j].drive == dd && g_shell_parts[j].index == i + 1)
            found = 1;
        }
        if (!found) {
          next_idx = i;
...
static void cmd_fdisk(char **argv, int argc) {
  ...
  if (drive < 4 && ata_drive_sectors(drive))
    valid = 1;
  if (drive >= SATA_DRIVE_BASE && ahci_drive_sectors(drive - SATA_DRIVE_BASE))
    valid = 1;
  if (!valid) {
...
static void rtlcfg_prompt_ip(const char *label, uint8_t *field) {
  char buf[32];
  terminal_printf("  %s [%d.%d.%d.%d]: ", label, field[0], field[1], field[2],
                  field[3]);
  readline(buf, 32);
  if (buf[0] == 0)
    return;
...
static void cmd_setup(void) {
  ...
  terminal_printf("  Current hostname: %s\n  New hostname (Enter to keep): ",
                  hostname);
  char buf[64];
  readline(buf, 64);
  if (buf[0]) {
  ...
  for (int d = 0; d < AHCI_MAX_PORTS; d++)
    if (ahci_drive_sectors(d))
      has_disk = 1;
  if (has_disk) {
    terminal_printf("  drives detected. Run 'install' to install to disk.\n");
  } else {
...
static void execute(char **argv, int argc) {
  ...
  if (scmp(argv[0], "emacs") == 0) {
    terminal_printf("stinger: emacs would take up all 4MB of RAM. Use nano.\n");
    return;
  }
  if (scmp(argv[0], "python") == 0 || scmp(argv[0], "python4") == 0) {
    terminal_printf(">>> print(zis is not python PC papa. But respect)\n>>> "
                    "Type 'help' for commands.\n");
    return;
  }
  ...
  if (scmp(argv[0], "windows") == 0) {
    terminal_printf(
        "stinger: this is not windows. there are no forced updates here.\n");
    return;
  }
  if (scmp(argv[0], "no") == 0) {
  ...
  if (scmp(argv[0], "gm") == 0) {
    terminal_printf("good morning. the kernel has been awake since boot.\n");
    return;
  }
  if (scmp(argv[0], "gn") == 0) {
  ...
  if (scmp(argv[0], "yaoigui") == 0) {
    ...
    terminal_printf("[stinger] Launching YaoiGUI compositor — type 'exit' or "
                    "Shut Down to return\n");
    yaoigui_run();
  }
...
static void print_banner(void) {
  ...
  terminal_writestring("\n"
                       ...
                       "\n");
  terminal_setcolor(terminal_make_color(VGA_COLOR_LIGHT_GREY, VGA_COLOR_BLACK));
  terminal_writestring("  stinger OS v4.0 | 101% shitty code | 140+ commands | "
