...
static float sgl_floorf(float x) { int t = (int)x; return (float)t - (x < (float)t ? 1.0f : 0.0f); }
...
int sgl_init(int width, int height) {
    ...
    g_sgl->clear_color = 0xFF7FB3FF;
    g_sgl->clear_depth = 0.0f;
    g_sgl->depth_test  = 1;
    g_sgl->cull_face   = 1;
...
static float sgl_fetch_attrib_f(sgl_attrib_t *a, int vertex_idx, int component) {
    ...
    size_t stride_bytes = (size_t)(a->stride ? a->stride : a->size * 4);
    uint8_t *src = (uint8_t*)b->data + a->offset + (size_t)vertex_idx * stride_bytes + (size_t)component * 4;
    if ((src + 4) > ((uint8_t*)b->data + b->size)) return 0.0f;
...
static void sgl_draw_triangle(sgl_sv_t v0, sgl_sv_t v1, sgl_sv_t v2, sgl_texture_t *tex) {
    ...
    float inv_area2 = 1.0f / (area2 < 0.0f ? -area2 : area2);


    float px = (float)minx + 0.5f, py = (float)miny + 0.5f;
    float e0_row = (py - v0.sy)*dx01 - (px - v0.sx)*dy01;
    float e1_row = (py - v1.sy)*dx12 - (px - v1.sx)*dy12;
    float e2_row = (py - v2.sy)*dx20 - (px - v2.sx)*dy20;
    ...
    for (int y = miny; y <= maxy; y++) {
        ...
        for (int x = minx; x <= maxx; x++) {
            ...
            if (e0 >= 0.0f && e1 >= 0.0f && e2 >= 0.0f) {
                ...
                float invw = v0.invw*w1 + v1.invw*w2 + v2.invw*w0;
                ...
                {
                    float u_w = v0.u_over_w*w1 + v1.u_over_w*w2 + v2.u_over_w*w0;
                    float v_w = v0.v_over_w*w1 + v1.v_over_w*w2 + v2.v_over_w*w0;
                    float sh_w = v0.shade_over_w*w1 + v1.shade_over_w*w2 + v2.shade_over_w*w0;
