...
void bin_ip(char**,int,sfs_node_t*);
...
void bin_gcc(char**,int,sfs_node_t*);
void bin_play(char**,int,sfs_node_t*);



static inline void bin_python_wrap(char**argv,int argc,sfs_node_t*cwd){
    (void)cwd;

    if(argc>=3){const char*a=argv[1];if(a[0]=='-'&&a[1]=='c'&&a[2]==0){cmd_python_c(argv,argc);return;}}
    cmd_python(argv,argc);
}
static inline void bin_pip_wrap(char**argv,int argc,sfs_node_t*cwd){
    (void)cwd; cmd_pip(argv,argc);
}
...
static const bin_entry_t bin_table[] = {
    ...
    {"python",   bin_python_wrap},
    {"python3",  bin_python_wrap},
    {"py",       bin_python_wrap},
    {"pip",      bin_pip_wrap},
    {"pip3",     bin_pip_wrap},
