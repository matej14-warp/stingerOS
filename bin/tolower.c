/* scorpion OS - bin/tolower.c */
#include "_bin_common.h"
void bin_tolower(char**argv,int argc,sfs_node_t*cwd){
    ...
    if(!d){terminal_printf("tolower: no input\n");return;}
    for(size_t i=0;i<sz;i++) terminal_putchar((char)b_tolower((char)d[i]));
    kfree(d);
}
