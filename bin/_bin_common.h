/* scorpion OS - bin/_bin_common.h
   Included by every bin/*.c file.
   Provides: standard includes, shared helpers (b_scmp, b_slen, b_itoa,
   b_printf, b_atoi, b_memset, b_memcpy), forward-declared external APIs,
   and the bin_fn_t signature so each file compiles standalone.           */
...
/* write unsigned int to buf, return ptr past end */
...
/* Simple b_printf to terminal (subset: %d %u %s %c %x %X %%) */
static inline void b_printf(const char *fmt, ...) {
    /* use __builtin_va_list for freestanding */
...
/* Shorthand for terminal_printf (most bin files already use terminal_printf) */
...
/* read a file node into a malloc'd buffer; caller must kfree; returns size or -1 */
...
/* iterate lines in a text buffer, calling cb(line, ctx) for each */
...
/* simple word-wrap: count chars to width */
