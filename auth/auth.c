...
static void k_strncpy(char *d, const char *s, size_t n) {
  size_t i = 0;
  while (i < n - 1 && s[i]) {
    d[i] = s[i];
    i++;
...
static void k_memset(void *d, uint8_t v, size_t n) {
  uint8_t *p = d;
  while (n--)
    *p++ = v;
}
...
int auth_verify(const char *username, const char *password, auth_user_t *out) {
  ...
  if (!pw_file || pw_file->type != SFS_FILE || !pw_file->data)
    return 0;
...
int auth_add_user(const char *username, const char *password, uint32_t uid,
                  uint32_t gid, const char *home, const char *shell) {
  sfs_node_t *etc = sfs_resolve(sfs_root(), "/etc");
  if (!etc)
    return -1;
  ...
  if (old_len && pw_file->data) {
    for (size_t i = 0; i < old_len; i++)
      buf[i] = ((char *)pw_file->data)[i];
  }
  for (size_t i = 0; i < nl; i++)
    buf[old_len + i] = new_line[i];
  buf[new_total] = 0;
...
void auth_init_etc(void) {
  ...
  if (!sfs_find_child(etc, "hostname"))
    sfs_write_file(etc, "hostname", (const uint8_t *)"stinger", 9);
  ...
  if (!root_home)
    sfs_mkdir(sfs_root(), "root");


  if (!sfs_find_child(etc, "gui_autostart"))
    sfs_write_file(etc, "gui_autostart", (const uint8_t *)"1", 1);
}
...
void login_prompt(void) {
  ...
  {
    sfs_node_t *etc = sfs_resolve(sfs_root(), "/etc");
    sfs_node_t *pw = etc ? sfs_find_child(etc, "passwd") : 0;
    int user_lines = 0;
    if (pw && pw->data) {
      const char *p = (const char *)pw->data;
      while (*p) {
    ...
    if (user_lines <= 1) {
      ...
      k_strncpy(current_user.username, "root", AUTH_NAME_MAX);
      k_strncpy(current_user.home, "/root", AUTH_HOME_MAX);
      k_strncpy(current_user.shell, "/bin/sh", AUTH_SHELL_MAX);
      current_user.uid = 0;
      current_user.gid = 0;
