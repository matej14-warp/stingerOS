/* scorpion OS - fetch/fetch.c
   scorpion package manager - like pacman but uses cdn.voided.uk
   Usage: fetch <package>
   Flow:
     1. HTTP GET cdn.voided.uk/fetch3/<package>
     2. Save .zip to /tmp/<package>.zip
     3. Extract to /tmp/<package>/
     4. Show contents + prompt user Y/N
     5. If Y: run setup-runtime.mbf from extracted dir  */
...
/* ---- ZIP extract callback ---- */
typedef struct {
    sfs_node_t *target_dir;
    int         file_count;
    char        has_setup;   /* 1 if setup-runtime.mbf was found */
} extract_ctx_t;
...
static void on_extracted_file(const char *fname, const uint8_t *data,
    ...
    /* Handle sub-paths: create directories as needed */
    char path[256];
    scpy(path, fname);

    /* Find if there's a directory component */
    ...
    if(last_slash) {
        *last_slash = 0;
        ...
        /* If fname ends in / it's a directory */
        ...
        /* No slash - file directly in root of zip */
        if(path[0]==0) return; /* skip empty */
...
extern char shell_getchar(void); /* provided by shell */
...
int fetch_package(const char *pkg_name) {
    ...
    terminal_printf("\n");
    terminal_printf(":: scorpion package manager\n");
    terminal_printf(":: fetching package: %s\n", pkg_name);
    terminal_printf(":: CDN: cdn.voided.uk\n\n");

    /* Build URL path: /fetch3/<package> */
    char url_path[256];
    ...
    /* Check it looks like a ZIP (magic PK\x03\x04) */
    if(body_len < 4 || body[0]!='P' || body[1]!='K') {
        terminal_printf("[fetch] error: response is not a ZIP file\n");
        terminal_printf("[fetch] (server may have returned an error page)\n");
        kfree(body);
        return -1;
    }

    /* Create /tmp/<pkg> extraction directory */
    ...
    /* Save raw zip */
    sfs_write_file(tmp_root, /* tmp/<pkg>.zip */ pkg_name, body, body_len);
    ...
    /* Run setup-runtime.mbf from the extracted directory */
...
void fetch_update_index(void) {
    ...
    /* Write index to /tmp/fetch-index */
