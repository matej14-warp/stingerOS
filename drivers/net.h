...
/* Public API */
void net_init(void);
void net_set_mac(const uint8_t mac[6]);   /* set MAC from active driver */
int  net_dhcp(void);
int  net_dhcp_once(void);   /* single fast attempt for boot */
void net_get_config(net_config_t *cfg);
void net_set_poll_fn(void (*fn)(void));
void net_set_send_fn(int  (*fn)(const uint8_t*, size_t));
void net_set_config(const net_config_t *cfg);   /* manual static config */
...
/* Set to 1 to cancel a blocking net operation (DHCP etc). Cleared automatically. */
extern volatile int g_net_cancel;

/* UDP raw send/recv for DNS module */
