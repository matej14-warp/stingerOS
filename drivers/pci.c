/* scorpion OS - drivers/pci.c
   Full PCI bus scan: all 256 buses × 32 devices × 8 functions.
   Reads vendor/device, class, BARs, and IRQ line.
   Results stored in g_pci_devices[].                           */
...
void pci_enable_busmaster(uint8_t bus, uint8_t dev, uint8_t fn) {
    ...
    pci_write(bus, dev, fn, 0x04, cmd | 0x07);  /* I/O + MEM + BusMaster */
}
