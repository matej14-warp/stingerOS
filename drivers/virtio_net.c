/* scorpion OS - drivers/virtio_net.c
   VirtIO network device driver (legacy virtio 0.9 / transitional 1.0).

   QEMU: -device virtio-net-pci
   PCI vendor/device: 0x1AF4 / 0x1000 (legacy) or 0x1041 (modern)

   Implementation:
   - PCI discovery and I/O BAR setup
   - Virtqueue initialisation (receive + transmit queues)
   - Polling-based send/receive (no IRQ needed for basic operation)
   - Plugs into the net stack via the same rndis shim interface        */
...
/* Vring sizes */
#define VRING_RX_SIZE  64
#define VRING_TX_SIZE  64
...
/* tx used ring not needed for polling-only TX */
...
int virtio_net_init(void){
    /* Scan PCI for VirtIO net (vendor 0x1AF4, dev 0x1000 or 0x1041) */
    for(uint8_t bus=0;bus<8;bus++)
        for(uint8_t dev=0;dev<32;dev++){
            ...
            /* Check subsystem: must be net (subsystem device 1) */
            ...
            /* Init sequence */
            outb_v(vio_base+VIRTIO_PCI_STATUS,0);  /* reset */
...
int virtio_net_send(const uint8_t *data, size_t len){
    ...
    outw_v(vio_base+VIRTIO_PCI_QUEUE_NOTIFY,1); /* notify TX queue */
    tx_idx++;
...
void virtio_net_poll(void){
    ...
    outw_v(vio_base+VIRTIO_PCI_QUEUE_NOTIFY,0); /* kick RX */
    (void)used_idx;
}
...
/* NE2000 has been moved to drivers/ne2000.c */

/* NE2000 registers — kept here as dead comment block, see ne2000.c */
#if 0
/* NE2000 registers */
#define NE_CMD   0x00
...
#define NE_CMD_RD2   0x20  /* abort/complete DMA */
...
int ne2000_init(void){
    ...
    /* ISA fallback — only if PCI scan found nothing.
       Use a stricter probe: reset the chip, wait for RST bit in ISR,
       then verify we can read back a known value via DCR to confirm
       a real NE2000 is present and not a ghost I/O address. */
    if(!io){
        uint16_t probes[]={0x300,0x280,0x320,0x340,0};
        for(int i=0;probes[i];i++){
            uint16_t base=probes[i];
            /* Step 1: issue reset via the reset port */
            outb_v(base+NE_RESET, inb_v(base+NE_RESET));
            for(volatile int j=0;j<50000;j++);
            /* Step 2: RST bit (0x80) must appear in ISR */
            if(!(inb_v(base+NE_ISR) & 0x80)) continue;
            /* Step 3: stop the chip and try to write/read DCR to
               confirm real NE2000 register file (not floating bus) */
            outb_v(base+NE_CMD, NE_CMD_PAGE0|NE_CMD_STOP|NE_CMD_RD2);
            for(volatile int j=0;j<5000;j++);
            outb_v(base+0x0E, 0x49);   /* DCR: 16-bit mode */
            uint8_t dcr_back = inb_v(base+0x0E);
            /* On a ghost address dcr_back is typically 0xFF or 0x00;
               a real NE2000 should return 0x49 or a close variant. */
    ...
    ne_out(NE_ISR,0xFF);
    ne_out(NE_DCR, 0x49); /* 16-bit, fifo thresh 8 bytes */
    ne_out(NE_RBCR0,0); ne_out(NE_RBCR1,0);
    ne_out(NE_RCR,0x20); /* monitor */
    ne_out(NE_TCR,0x02); /* loopback */
    ne_out(NE_PSTART,NE_RX_START); ne_out(NE_PSTOP,NE_RX_STOP);
    ne_out(NE_BNRY,NE_RX_START);
    ne_out(NE_TPSR,NE_TX_PAGE);
    ne_out(NE_IMR,0x00);

    /* Read MAC via remote DMA from PROM at address 0 */
    uint8_t prom[32];
...
#endif /* 0 - ne2000 moved to ne2000.c */
