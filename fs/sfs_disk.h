...
/* Auto-persistence API */
void sfs_set_persist_target(int drive, uint32_t lba);
void sfs_persist(void);           /* mark dirty, debounced flush */
void sfs_persist_flush(void);     /* call from idle loop to actually write */
void sfs_persist_suppress(int s); /* 1=suppress auto-persist, 0=re-enable */
