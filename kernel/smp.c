...
volatile int    g_ap_work_done        = 1;
static volatile int g_ap_parallel_sync = 0;


static inline int atomic_fetch_add(volatile int *p, int v) {
    __asm__ volatile ("lock xaddl %0, %1" : "+r"(v), "+m"(*p) : : "memory");
    return v;
}

static inline void atomic_dec(volatile int *p) {
...
void lapic_init(void) {
    ...
    lapic_write(LAPIC_SVR, svr | 0x100 | 0xFF);


    if (lapic_get_id() == g_cpus[0].apic_id) {
...
static uint8_t ap_stack[SMP_MAX_CPUS][AP_STACK_SIZE] __attribute__((aligned(16)));

static void write_ap_trampoline(uint32_t paddr, int cpu_idx) {
    ...
    uint32_t stack_top = (uint32_t)(uintptr_t)(ap_stack[cpu_idx] + AP_STACK_SIZE);
...
void smp_boot_ap_start(uint8_t apic_id, uint32_t trampoline_page) {
    ...
    for (int c = 0; c < g_cpu_count; c++) {
        if (g_cpus[c].apic_id == apic_id) { cpu_idx = c; break; }
    }
    write_ap_trampoline(paddr, cpu_idx);
...
int smp_boot_ap_finish(uint8_t apic_id) {
    ...
    uint32_t timeout = 100; // max 100 ms wait per AP in finish
    for (int c = 0; c < g_cpu_count; c++) {
        if (g_cpus[c].apic_id == apic_id) {
            while (!g_cpus[c].online && timeout--) sleep_ms(1);
...
int smp_boot_ap(uint8_t apic_id, uint32_t trampoline_page) {
    smp_boot_ap_start(apic_id, trampoline_page);
    sleep_ms(10);
    return smp_boot_ap_finish(apic_id);
}
...
int smp_dispatch_ap(void (*fn)(void*), void *arg, int wait) {
    ...
    if (!wait && !g_ap_work_done) return -1;
...
void smp_parallel_for(void (*fn)(void *, int, int), void *arg, int total_iterations) {
    ...
    if (g_cpu_count <= 1) {
        fn(arg, 0, total_iterations);
        return;
    }

    static parallel_task_t job;
    job.fn = fn;
    job.arg = arg;
    job.next_iteration = 0;
    job.total_iterations = total_iterations;
    job.grain_size = (total_iterations > 100) ? 4 : 1;
    ...
    if (active_cpus > 1) active_cpus--;
    job.remaining_cpus = active_cpus;


    g_ap_work_fn = (ap_work_fn_t)0x1;
    g_ap_work_arg = &job;
    g_ap_work_done = 0;


    int notified = 0;
    for (int i = 0; i < g_cpu_count && notified < active_cpus - 1; i++) {
        if (!g_cpus[i].is_bsp && g_cpus[i].online) {
            lapic_send_ipi(g_cpus[i].apic_id, IPI_FIXED | 0x40);
            notified++;
        }
    }
    ...
    while (1) {
        int start = atomic_fetch_add(&job.next_iteration, job.grain_size);
        if (start >= job.total_iterations) break;
        int end = start + job.grain_size;
        if (end > job.total_iterations) end = job.total_iterations;
        fn(arg, start, end);
    }

    atomic_dec(&job.remaining_cpus);


    while (job.remaining_cpus > 0) __asm__ volatile ("pause");
...
void __attribute__((noreturn)) smp_ap_entry(void) {
    ...
    for (;;) {
        ...
        if (g_ap_work_fn) {
            if (g_ap_work_fn == (ap_work_fn_t)0x1) {
                parallel_task_t *job = (parallel_task_t *)g_ap_work_arg;
                while (1) {
                    int start = atomic_fetch_add(&job->next_iteration, job->grain_size);
                    if (start >= job->total_iterations) break;
                    int end = start + job->grain_size;
                    if (end > job->total_iterations) end = job->total_iterations;
                    job->fn(job->arg, start, end);
                }
                atomic_dec(&job->remaining_cpus);
                while (g_ap_work_fn == (ap_work_fn_t)0x1) __asm__ volatile ("pause");
            } else {
